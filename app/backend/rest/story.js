...
self.prototype.patch = async function(){
	try{
		...
		const MAP = await this.mongodb.find('map',{ AUDIO: { $exists: true, $not: {$size: 0} } });
		for(let i=0;i<MAP.length;i++){
			const r = await this.mongodb.findOne(this.object,MAP[i].STORY);
			if(r!=null && MAP[i].AUDIO!=null){
				await this.mongodb.updateOne(this.object,r._id,{$set: {audio: MAP[i].AUDIO.replace('/media/audio/story/','/assets/media/audio/story/')}});		
			}
		}*/
	}catch(e){
		console.log(e);
