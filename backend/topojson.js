...
self.prototype.start = async function(){
	try{
		...
		parche = await mongodb.find("topojson",{name: 'regiones'});
		...
		parche = await mongodb.find("topojson",{name: 'comunas'});
...
self.prototype.total = async function(req,res){
	try{
		...
		let total = await mongodb.count('topojson',query);
		res.send({data: total});
	}catch(e){
...
self.prototype.collection = async function(req,res){
	try{
		...
		let data = await mongodb.find('topojson',query,options);
		res.send({data: data});
	}catch(e){
		res.send({data: null,error: e.toString()});
	}
...
self.prototype.read = async function(req,res){
	try{
		let row = await mongodb.findOne('topojson',req.params.id);
		res.send({data: row});
	}catch(e){
		res.send({data: null,error: e.toString()});
	}
