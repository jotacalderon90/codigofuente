...
self.prototype.getMasterContent = function(masterName){
	try{
		return fs.readFileSync(config.dir + config.properties.views + masterName + ".html",'utf8');
	}catch(e){
		this.error = e;
		return undefined;
	}
...
self.prototype.processInclude = function(process_template){
	...
	var includeTemplate = "";
	try{
		includeTemplate = fs.readFileSync(config.dir + config.properties.views  + includeName + ".html",'utf8');
	}catch(e){
