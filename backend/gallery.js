...
const self = function(){
	...
	this.dir = config.dir + "/frontend" + this.frontpath;
}
...
self.prototype.index = function(req,res,next){
	res.render("index",{...helper.toRender(req), onOpen: {app: "gallery", action: "open"}});
}
	...
	try{
		...
		if(Array.isArray(req.files.file)){
			for(let i=0;i<req.files.file.length;i++){
				...
				await helper.upload_process(req.files.file[i], this.dir + req.params.id + '/' + req.files.file[i].name);
			}
		}else{
			console.log("subiendo archivo" + req.files.file.name);
			await helper.upload_process(req.files.file, this.dir + req.params.id + '/' + req.files.file.name);
		}
